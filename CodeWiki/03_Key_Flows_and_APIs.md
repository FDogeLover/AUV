...
- **入口 `main()`**
  - [main.c](file:///workspace/ANO_LX_FC_倾角保护版/FcSrc/main.c#L22-L33)
  - 调用顺序：`All_Init()` → `Scheduler_Setup()` → `while(1) Scheduler_Run()`

- **整机初始化 `All_Init()`**
  - T265版：[Drv_BSP.c](file:///workspace/ANO_LX_FC_T265代替光流/DriversBsp/Drv_BSP.c#L21-L89)
...
- `sched_task_t`：函数指针 + 频率 + 周期tick + 上次运行时间
  - [Ano_Scheduler.h](file:///workspace/ANO_LX_FC_T265代替光流/FcSrc/Ano_Scheduler.h#L4-L10)
- `Scheduler_Setup()`：把 `rate_hz` 转换为 `interval_ticks = TICK_PER_SECOND / rate_hz`
  - [Scheduler_Setup](file:///workspace/ANO_LX_FC_倾角保护版/FcSrc/Ano_Scheduler.c#L81-L95)
- `Scheduler_Run()`：读取 `GetSysRunTimeMs()`，到期则执行任务
  - [Scheduler_Run](file:///workspace/ANO_LX_FC_倾角保护版/FcSrc/Ano_Scheduler.c#L97-L116)

系统时基：

- `TICK_PER_SECOND = 1000`（1ms tick）：[SysConfig.h](file:///workspace/ANO_LX_FC_T265代替光流/FcSrc/SysConfig.h#L13-L16)

### 3.2.2 1ms 主任务（更偏实时）

在 STM32F407 平台上，通过 TIM7 中断直接驱动 `ANO_LX_Task()`：

- [stm32f4xx_it.c](file:///workspace/ANO_LX_FC_T265代替光流/DriversMcu/STM32F407/Drivers/stm32f4xx_it.c#L59-L69)
...
- `_rc_input_st`：PPM/SBUS 原始通道、信号频率、failsafe等
  - [Drv_BSP.h](file:///workspace/ANO_LX_FC_T265代替光流/DriversBsp/Drv_BSP.h#L7-L33)
...
- 定义见 [ANO_LX.h](file:///workspace/ANO_LX_FC_T265代替光流/FcSrc/ANO_LX.h#L37-L53)

典型赋值路径：

- RC摇杆 → `RC_Data_Task()` → `rt_tar`（并标记 `dt.fun[0x41].WTS = 1`）
  - T265版示例：[ANO_LX.c](file:///workspace/ANO_LX_FC_T265代替光流/FcSrc/ANO_LX.c#L64-L185)
...
- 模式命令/状态、解锁命令/状态
- IMU就绪/起飞/在空/降落标记
- 定义见 [LX_FC_State.h](file:///workspace/ANO_LX_FC_T265代替光流/FcSrc/LX_FC_State.h#L22-L41)

状态机执行入口：

- `LX_FC_State_Task(dT_s)`：[LX_FC_State.c](file:///workspace/ANO_LX_FC_T265代替光流/FcSrc/LX_FC_State.c#L164-L174)
...
- `FC_data`：飞行状态、控制模式、位置/航向、电池等
  - 定义：[User_ComBoard.h](file:///workspace/ANO_LX_FC_T265代替光流/FcSrc/User_ComBoard.h#L29-L55)
...
- [ANO_LX_Data_Exchange_Task](file:///workspace/ANO_LX_FC_T265代替光流/FcSrc/ANO_DT_LX.c#L478-L525)

### 3.4.2 T265版：自定义上行帧 `t265_data_send()`

T265版在匿名协议基础上，构造 ID=0xF1 的“灵活数据帧”，用于把任务/视觉/控制输出等信息回传上位机：

- [t265_data_send](file:///workspace/ANO_LX_FC_T265代替光流/FcSrc/ANO_DT_LX.c#L527-L580)
...
- **灵活帧发送**（Fletcher-8校验）
  - `flex_send()`：[my_protocol.c](file:///workspace/ANO_LX_FC_倾角保护版/Mycode/my_protocol.c#L37-L61)
  - `flex_send_t265_vel()`（0xF1）与 `flex_send_guangliu_vel()`（0xF2）：[my_protocol.c](file:///workspace/ANO_LX_FC_倾角保护版/Mycode/my_protocol.c#L63-L79)
- **Pi→FC 解析入口**
  - `pi_receive(u8 data)`：支持两类帧
    - 0x01：T265速度帧（vx/vy/yaw）
    - 0x02：飞行指令帧（task_sta、com_x/y/z/yaw、next_task、sp_side）
  - 见：[my_protocol.c](file:///workspace/ANO_LX_FC_倾角保护版/Mycode/my_protocol.c#L92-L161)
