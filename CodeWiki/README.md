...
1. 固件整体入口：[main.c](file:///workspace/飞控固件/FcSrc/main.c#L22-L37)（两工程一致）
2. 板级初始化：`All_Init()`：[Drv_BSP.c](file:///workspace/飞控固件/DriversBsp/Drv_BSP.c#L21-L89)
3. 运行骨架：
   - 裸机调度器：`Scheduler_Setup/Run()`：[Ano_Scheduler.c](file:///workspace/飞控固件/FcSrc/Ano_Scheduler.c#L65-L116)
   - STM32F407 定时中断驱动主任务：`TIM7_IRQHandler -> ANO_LX_Task()`：[stm32f4xx_it.c](file:///workspace/飞控固件/DriversMcu/STM32F407/Drivers/stm32f4xx_it.c#L59-L69)
4. 飞控 1ms 主任务（T265版）：`ANO_LX_Task()`：[ANO_LX.c](file:///workspace/飞控固件/FcSrc/ANO_LX.c#L289-L341)
5. 树莓派端串口协议与任务入口：
