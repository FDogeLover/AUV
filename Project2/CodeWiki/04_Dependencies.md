...
- **Keil uVision / ARMCC 5.06**（工程文件显示使用 ARM-ADS 工具链）
  - STM32F407 工程：[ANO_LX_STM32F407.uvprojx](file:///workspace/ANO_LX_FC_T265代替光流/ProjectSTM32F407/ANO_LX_STM32F407.uvprojx#L10-L15)
  - MSP432 工程：[ANO_LX_MSP432.uvprojx](file:///workspace/ANO_LX_FC_T265代替光流/ProjectMSP432/ANO_LX_MSP432.uvprojx#L10-L15)
- **STM32F4 CMSIS / StdPeriph**
  - 位于 [DriversMcu/STM32F407/Libraries](file:///workspace/ANO_LX_FC_T265代替光流/DriversMcu/STM32F407/Libraries)
- **USB Stack（STM32F407）**
  - 位于 [DriversMcu/STM32F407/Libraries/USBStack](file:///workspace/ANO_LX_FC_T265代替光流/DriversMcu/STM32F407/Libraries/USBStack)
- **CMSIS DSP / arm_math**
  - STM32F407 目录中包含 DSP_Lib 源码：见 [DriversMcu/STM32F407/Libraries/CMSIS/DSP_Lib](file:///workspace/ANO_LX_FC_T265代替光流/DriversMcu/STM32F407/Libraries/CMSIS/DSP_Lib)
- **TI driverlib（MSP432/TM4C）**
  - MSP432 driverlib（含多编译器 Makefile）：[driverlib/gcc/Makefile](file:///workspace/ANO_LX_FC_倾角保护版/DriversMcu/MSP432P401/Drivers/ti/devices/msp432p4xx/driverlib/gcc/Makefile#L28-L47)
...
- `FcSrc/*` 依赖 `SysConfig.h`，进一步依赖 `McuConfig.h`（平台差异）与 `Drv_BSP.h`（板级能力聚合）
  - 例如：[SysConfig.h](file:///workspace/ANO_LX_FC_T265代替光流/FcSrc/SysConfig.h#L1-L5)
- `Drv_BSP.c` 通过调用 `DriversMcu/*/Drivers/Drv_*.c` 暴露的统一接口完成初始化
  - 例如：`DrvUart*Init/DrvTimerFcInit/DrvPwmOutInit` 等：[Drv_BSP.c](file:///workspace/ANO_LX_FC_T265代替光流/DriversBsp/Drv_BSP.c#L21-L89)
...
- `TIM7_IRQHandler` 直接调用 `ANO_LX_Task()`：[stm32f4xx_it.c](file:///workspace/ANO_LX_FC_T265代替光流/DriversMcu/STM32F407/Drivers/stm32f4xx_it.c#L59-L69)
- `ANO_LX_Task()` 串联：
  - RC输入：`DrvRcInputTask`（BSP）
  - 状态机：`LX_FC_State_Task`（FcSrc）
  - 通信：`ANO_LX_Data_Exchange_Task`（FcSrc）
  - 输出：`DrvMotorPWMSet`（MCU驱动）
  - 见：[ANO_LX_Task](file:///workspace/ANO_LX_FC_T265代替光流/FcSrc/ANO_LX.c#L289-L341)
