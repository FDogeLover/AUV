...
运行（先确保已 pip install pytest pyserial）：
    cd drone_control/circle_pole && python -m pytest test_land_logging.py -v
"""
import io
import json
import os
...
class TestLandUnlockPwmConfirm:
    """2026-07-10矩形路径到达确认耗时基线测试复现了问题7描述的假阳性：终端打印
    "降落确认：已上锁"、用户确认是人工接管完成的降落。核对飞行日志发现`unlock_sta`
    确实连续读到0满足了去抖要求，但同一时刻`motor_pwm_mask`全程稳定为15(四电机都
    还在出PWM)，两个字段直接矛盾——说明只看`unlock_sta`的去抖不够，需要同时要求
    `motor_pwm_mask==0`才能真正确认已上锁。`motor_pwm_mask`不可用(None，比如还没
    收到过帧2)时不应阻塞确认，退化成只看`unlock_sta`的旧行为，避免因诊断字段缺失
    导致永远无法确认。"""
    ...
    def test_unlock_confirmed_but_motor_pwm_nonzero_does_not_confirm(self, monkeypatch):
        ...
        m = _make_mission_for_land()
        m._log_file = io.StringIO()
        # unlock_sta连续读到0(去抖本身满足)，但motor_pwm_mask全程保持0x0F(电机仍在出PWM)
    ...
    def test_motor_pwm_mask_unavailable_falls_back_to_unlock_only(self, monkeypatch):
        ...
        m._log_file = io.StringIO()
        m.re_fc = TransientUnlockList([0] * 14, seq=[0, 0, 0, 0, 0, 0, 0, 0])
        # serial_fc_ref为None(默认)，land()里motor_pwm_mask会是None，不应阻塞确认
