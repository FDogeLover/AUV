...
背景：2026-07-10真机降落回归测试发现，降落末尾激光传感器偶发返回类似
0xFFFFFFFF的错误码，`_last_laser_height_cm`除以100后变成约4.29e7米的
垃圾值。原逻辑(loop()/land()里)只判断 laser_h > 0.05，没有上限，垃圾值
被直接当真实高度写进 pos[2]/land_pos[2]，污染飞行日志的高度曲线。

运行（先确保已 pip install pytest pyserial）：
    cd drone_control/circle_pole && python -m pytest test_laser_height_valid.py -v
"""
import os
import sys
