...
| `17_v2_enhanced.png` | 在16号基础上进一步尝试接近前置UVC摄像头的观感：黑电平+白平衡+百分位数自动拉伸对比度+饱和度提升1.35倍+双边滤波降噪+反锐化蒙版锐化，木纹暖色调更明显、对比度更好，但仍能看出比UVC自带ISP噪点更多(增益放大了传感器噪声) |
| `_crop_debug.png` / `_crop_grid.png` | 前置摄像头FOV标定(`08_fov_calib_box15cm_at50cm.jpg`)时用的像素网格标尺辅助图，用来读箱子边缘对应的像素坐标，反推`tan(半角)=W/(2D)`里的像素宽度 |
