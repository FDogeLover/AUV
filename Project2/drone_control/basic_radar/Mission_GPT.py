...
YAW_TEST_KP = float(os.getenv("DRONE_YAW_TEST_KP", "0.4"))
...
# 均收敛。但同一天用Kp=0.45做"默认行为"复测时(不带环境变量)出现了跟第一次
# 0.45样本不一致的结果——峰值10.38°未收敛，现场观察到可见旋转，说明0.45已经
# 比较接近边界、run-to-run方差偏大；Kp=0.4的唯一样本(峰值7.74°有界震荡)
# 全程未观察到可见旋转，更保守。现将Kp=0.4设为默认值——设置环境变量
# DRONE_YAW_TEST_KP=0 可临时回退到旧的"喂弧度，不修正"安全状态(应急回滚用)。
# 每组增益目前都只有1~2次真机样本，尚未充分验证可重复性，如果真机观察到异常
# 应立即回退到0并重新评估。
