...
| 16 | navigate() yaw修正回路 | 🟡/首轮A/B通过 | 2026-07-18定位旧navigate()额外`-vyaw`把负反馈变正反馈；新航向保持完整路线开启组净yaw变化绝对值较关闭组降约60%，但65.8%导航记录处于±1°/s非零限幅、全范围仍10.94°，保持默认关闭继续复测 |
...
| 27 | fire_patrol长直线横向偏差：掠过切角+yaw残余耦合 | 🟡/A/B已测 | 15cm判据已解决0.4m提前切角；开启航向保持后长直线横向绝对误差均值/中位/P95分别改善22%/27%/20%，但最坏峰值0.1383→0.1444m略差，结论仅为典型波动改善、最坏值未改善 |
...
| 38 | fire_patrol安全结束后串口监听线程关闭竞态 | 🟡/低优先级 | 两轮2026-07-18完整路线均在上锁、任务结束、T265/串口关闭后触发`listen_fc`线程异常并退出码1；不影响飞行，根因是`listen_end()`不等待线程且部分`read()`未捕获关闭异常，待桌面回归修复 |
