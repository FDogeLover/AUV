...
u8 RxBuffer[256];//
u8 LidarBuffer[256];//
u8 pi_receive_done_sign=0;//¦Ë
u8 lidar_receive_done_sign=0;//¦Ë
s16 CSPX=0,CSPY=0;
u8 x_high=0;
u8 x_low=0;
u8 y_high=0;
...
s16 t265_yaw_angle = 0; // T265 Æ«º½½Ç£¬µ¥Î» 0.01¡ã
...
void pi_receive(u8 data)//§¿ 2
{
	...
	else if(state_1==8&&data==0xFF)	//Ö¡Î²
	{
		RxBuffer[8]=data;
	...
	else if(state_1==17&&data==0xFF)	//¦Â
	{
		RxBuffer[9]=data;
