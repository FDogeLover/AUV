...
volatile _rt_tar_un rt_tar;
volatile _pwm_st pwm_to_esc;
_fc_bat_un fc_bat;
volatile _fc_att_un fc_att;
_fc_att_qua_un fc_att_qua;
volatile _fc_vel_un fc_vel;
...
static inline void RC_Data_Task(float dT_s)
{
	...
	if ( rc_in.fail_safe == 0)
	{
		...
		else
		{
			...
			mod_f[0] = 2;
			pi_ctrl_mode = 1;
			if(rc_in.rc_ch.st_data.ch_[ch_8_aux4] > 1200 && rc_in.rc_ch.st_data.ch_[ch_8_aux4] < 1700) //ҡ��һ������ʱ�л��ɹ���
				pi_ctrl_mode = 0;
		}
		//���л�ģʽʱִ��
		if (mod_f[1] != mod_f[0])
...
//		{		
		...
		if (pi_ctrl_mode != 1)
		rt_tar.st_data.yaw_dps = -tmp_ch_dz[ch_4_yaw] * 0.00238f * MAX_YAW_DPS; //��Ϊҡ�˺�����Ͷ���ĺ������෴������ȡ��		
		//��ݮ�ɿ���ģʽʱ��ʹ�� received_data ���� RC ֵ
//		if(pi_ctrl_mode == 1)
//		{
...
volatile u8 pi_ctrl_mode = 0;	//����ϵͳ��0=RC+����, 1=��ݮ��+T265
