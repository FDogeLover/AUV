...
class FlyController:
    def __init__(self,Gpio, route_file, serial_port="/dev/ttyS0", baudrate=230400,
...
def TargetChoose():
    ...
    try:
        ...
        print("打开串口失败: {e}")
    iindex = ser.readline() # 示例：要调用的索引
    route_file = file_mapping.get(index, "default.txt")  # 获取文件名，如果不存在则返回默认值
    return route_file

if __name__ == '__main__':
    route_file = TargetChoose()
    Gpio = GpioCtrl()
    fc = FlyController(Gpio,route_file)
    fc.main_loop()