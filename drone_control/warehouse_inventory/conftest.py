"""warehouse_inventory 模块的 pytest 配置和共享测试 fixtures。"""
import os
import sys
...
def expected_gpio_pin():
    """warehouse_inventory 模块使用 BCM17 作为按键默认引脚。"""
    return 17


@pytest.fixture
def expected_led_pins():
    """warehouse_inventory 模块的 LED 引脚映射。"""
    return {'R': 6, 'G': 25, 'B': 24}


@pytest.fixture
def mission_extra_kwargs():
    """warehouse_inventory 模块的 mission() 构造函数不需要额外参数。"""
    return {}
