...
class mission:
    ...
    def __init__(self, re_fc: List[int], se_fc: List[int],
                 ...
                 serial_fc_ref=None,
                 video_source=None, color_detector=None):
        self.re_fc = re_fc
        self.se_fc = se_fc
        ...
        # 植保模式
        self._plant_laser: Optional[LaserSpray] = None
        self._plant_grid_ids: list[int] = []  # 航点索引 → grid_id 映射
    ...
    def load_waypoints(self):
        ...
        config_path = os.getenv("PLANT_CONFIG")
        if config_path:
            from pathlib import Path
            from Lcode.grid_map import GridMap
            from Lcode.coverage_planner import plan_coverage_path
            gm = GridMap(Path(config_path))
            wp = plan_coverage_path(gm)
            if wp:
                # 建立航点索引 → grid_id 映射
                self._plant_laser = LaserSpray()
                all_spray = gm.spray_grids()
                # 用 coverage_planner 相同的逻辑找出 grid 顺序
                from Lcode.coverage_planner import _rows_containing_spray, _snake_from_start
                rows = _rows_containing_spray(gm)
                ordered = _snake_from_start(rows, gm.start_grid or 21)
                # waypoints[0]=home, waypoints[-1]=home, 中间对应 ordered grid
                self._plant_grid_ids = [0] + ordered + [0]
                logger.info(
                    f"植保模式：{len(wp)} 个航点，"
                    f"{len(gm.spray_grids())} 个播撒网格"
                )
                return wp
            logger.warning("植保模式：航线生成为空，回退默认航点")
    ...
    def loop(self):
        while self.task_running:
            ...
            # ── 颜色识别：每 ~100ms 抓一帧（不阻塞控制循环） ──
            self._color_frame_skip += 1
            if self._video_source is not None and self._color_detector is not None:
                if self._color_frame_skip >= 3:  # 30ms × 3 ≈ 100ms
                    self._color_frame_skip = 0
                    try:
    ...
    def _advance_waypoint(self, reason, pos, target, arrival_distance):
        ...
        # 植保模式：到达网格时触发激光笔闪烁
        if self._plant_laser is not None:
            if 0 <= completed_index < len(self._plant_grid_ids):
                gid = self._plant_grid_ids[completed_index]
                if gid > 0:
                    self._plant_laser.spray(grid_id=gid)
        ...
        if self._color_detector is not None:
            if 0 <= completed_index < len(self._plant_grid_ids):
                gid = self._plant_grid_ids[completed_index]
                if gid > 0:
                    detected, conf = self._color_detector.classify()
                    self._last_grid_color = (detected, conf)
                    logger.info(
        ...
        try:
            with self._log_lock:
                self._log_file.write(json.dumps({
                    ...
                    "grid_id": (
                        self._plant_grid_ids[target_index]
                        if 0 <= target_index < len(self._plant_grid_ids)
                        else None
                    ),
                    "detected_color": (
                        self._last_grid_color[0].name if self._last_grid_color[0] else None
                    ),
                    "color_confidence": round(self._last_grid_color[1], 3),
                }) + "\n")
                self._log_file.flush()
        except Exception:
            pass
