"""旧 yaw_pid 退役后的任务级航向保持回归测试的共享实现。

使用方式：在各子模块的 test_yaw_unit_fix.py 中导入：
    from shared_tests.test_yaw_unit_fix import (
        test_old_yaw_pid_is_removed,
        test_navigate_sends_controller_sign_without_extra_negation,
        test_small_error_inside_deadband_sends_zero,
    )
"""
import math
...
def _make_mission(**extra_kwargs):
    m = mission([0] * 14, [0] * 11, realsense_obj=None, serial_fc_ref=None, **extra_kwargs)
    m.t265_ok = True
