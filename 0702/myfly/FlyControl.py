...
class FlyController:
    def __init__(self, route_file='router.txt', serial_port="/dev/ttyS0", baudrate=230400):
        # 控制变量
        # self.Gpio = Gpio
        ...
        # 串口
        self.port = serial.Serial(port=serial_port, baudrate=baudrate, stopbits=1, parity=serial.PARITY_NONE, timeout=20)
    ...
    def SetFlag(self, index):
        self.LaserShotFlag1 = 1 if index in self.LaserIndex1 else 0
        self.LaserShotFlag2 = 1 if index in self.LaserIndex2 else 0
        self.CamearSpinFlag = 1 if index == 8 else 0
                ...
                if self._265Ready:
                    try:
                        ...
                        if pose:
                            ...
                            # 激光器控制
                            # if self.LaserShotFlag1:
                            #     self.Gpio.laser_set_1(0.5)
                            #     self.LaserShotFlag1 = 0
                            # if self.LaserShotFlag2:
                            #     self.Gpio.laser_set_2(0.5)
                            #     self.LaserShotFlag2 = 0
                            # # 舵机控制
                            # if self.CamearSpinFlag:
