...
```text
 ...
 CAR收到ACK后继续完成固定5秒安全倒计时，倒计时内不得驱动车轮
 -> 5秒安全倒计时结束后小车开始行进
 UAV红灯累计满5秒且无人机自身T265严格预检通过后开始起飞
 -> T265失败则取消任务，不等待终端人工输入
```

重复 `CAR_START` 不得重复起飞；session不匹配的旧包不得改变状态。
当前不新增第二阶段启动消息：小车按固定5秒倒计时启动；若无人机自身T265在5秒内
未完成严格预检，无人机必须延迟起飞或取消任务，不得为了追赶小车绕过预检。
