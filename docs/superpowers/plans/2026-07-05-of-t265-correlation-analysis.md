# 光流 vs T265 速度相关性分析脚本 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 写一个纯 stdlib 的离线诊断脚本，读取 `flight_data.jsonl`，计算 T265 速度与光流融合速度的同轴/交叉轴相关系数（含 ±5 采样点时延扫描），验证"光流坐标系可能相对机体旋转约90°"的假设。

**Architecture:** 单文件脚本 `drone_control/tools/analyze_of_t265_correlation.py`，无第三方依赖。核心是三个纯函数（解析、过滤、Pearson相关系数）+ 一个主流程（时延扫描 + 打印报告），用命令行参数指定日志路径和阈值。

**Tech Stack:** Python 3 标准库（`json`, `argparse`, `math`），无 numpy/pandas。
...
**Files:**
- Create: `drone_control/tools/analyze_of_t265_correlation.py`（脚本独立运行，不需要作为包导入，无需 `__init__.py`）

**背景**：日志文件路径 `ubuntu-pi:/home/sunrise/Desktop/FJJ/basic/flight_data.jsonl`，387行，每行样例：
```json
{"t": 1783149829.81, "state": "NAVIGATE", "target_idx": 0, "pos": [0.1928, 0.0634, 0.61], "target": [0.0, 0.0, 1.0], "vx": -9, "vy": -3, "vyaw": 0, "t265_vel": [-0.0414, -0.0066], "of1_vel_cms": [2, 0]}
```
没有 `t265_vel`/`of1_vel_cms` 字段的行（如 `{"event": "task_start"}`）需要跳过。

- [ ] **Step 1: 创建 tools 目录并写脚本**

创建文件 `drone_control/tools/analyze_of_t265_correlation.py`，完整内容：
