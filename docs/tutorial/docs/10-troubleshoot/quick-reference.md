...
| HOVER_WAIT后高度上不去 | 飞控/IMU状态复位 | 遥控接管；避免低空触发HOVER_WAIT | #26 |
| 一键降落没反应 | CMD通道指令撞车 | 已用DESCEND/HOVER_WAIT替代；遥控可接管 | #7, #46 |
| SSH连不上板子 | 动态IP变化 | 路由器后台查IP；接显示器检查 | — |
| Keil编译报错（编码） | 文件被UTF-8编辑器修改 | 人工：Keil内编辑；AI：edit_firmware.py 恢复 | git恢复 |
...
| 工程决策和调试记录 | `.Codex/memory/MEMORY.md`（AI内部记忆，可参考） |
| 待办事务 | `docs/TODO.md` |
...
| 代码文档 | `CodeWiki/` |
| 匿名上位机使用 | [匿名上位机使用](../09-workflow/ground-station.md) |
